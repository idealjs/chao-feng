...
class CommonGraph {
  ...
  public removeEdge(unit: IUnit<any[], any, any>, to: IUnit<any[], any, any>) {
    this.adjacency.set(
      unit,
      (this.adjacency.get(unit) ?? []).filter((node) => node.commonUnit !== to)
    );
  }
  ...
  public storeHasCircle() {
    try {
      const searchStatus = new WeakMap<IUnit<any[], any, any>, SEARCH_STATUS>();
      ...
      const searchStoreUnit = (unit: IUnit<any[], any, any>) => {
        ...
        if (searchStatus.get(unit) === SEARCH_STATUS.DOING) {
          throw new Error(`Circular dependency detected on unit ${unit}`);
        }
