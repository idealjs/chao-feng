  ...
  implements IUnit<Params, Done, Faild>
{
  ...
  constructor(effect: Effect<Params, Done, Faild>, unitOptions: IUnitOptions) {
    this.effect = effect;
    this.unitOptions = {
      scope: defaultScope,
      type: UNIT_TYPE.UNIT,
    ...
    if (unitOptions.id != null && this.scope.hasUnit(unitOptions.id)) {
      throw new Error(`Has already set with id: ${unitOptions.id.toString()} `);
    }
  ...
  on<TParams extends unknown[], TDone, TFaild>(
    ...
    const eventName =
      typeof listenerOrEvent === "function" ? updateSymbol : listenerOrEvent;
